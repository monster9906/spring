...
public interface AutowireCapableBeanFactory extends BeanFactory {
	...
	/**
	 * 按属性名称自动布线。 Spring寻找与需要自动装配的属性同名的bean。例如，如果一个bean定义被设置为按名称自动装配，并且包含一个master属性（即，它具有setMaster（..）方法），那么Spring将查找一个名为master的bean定义并使用它来设置该属性。
	...
	/**
	 * 如果容器中恰好存在一个属性类型的bean，则使该属性自动连接。如果存在多个错误，则将引发致命异常，这表明不能对该bean使用byType自动装配。如果没有匹配的bean，则什么都不会发生（未设置该属性）
	...
	/**
	 * 当不知道想要用以上那种自动装配注入方式注入时，就可以叫个容器自己来处理决定   构造器——>按照类型
